...
Define the Technical Design for TD-015 Exposure Engine.

Exposure Engine exists to generate explainable exposure recommendations from Overall Market Score.
...
This Technical Design covers Exposure Engine as a Portfolio Intelligence component.
...
- Internal workflow.
- Data contract ownership for Exposure Recommendation.
...
- The module must not calculate regime; calculate OMS; access Foundation.
...
Exposure Engine occupies the TD-015 position in that chain and preserves Regime First, Single Source of Truth, Separation of Concerns, Hard Gate Principle, Explainability, Auditability, and approved dependency direction.
...
Exposure Engine consumes approved upstream outputs and produces Exposure Recommendation for approved downstream consumers.

Purpose:

- Generate explainable exposure recommendations from Overall Market Score.

Responsibilities:

- Accept only declared upstream contracts.
- Produce only Exposure Recommendation.
...
- Exposure Engine does not own upstream contract creation.
- Exposure Engine does not bypass downstream module ownership.
- Exposure Engine does not access Foundation directly.
- Exposure Engine does not modify immutable upstream outputs.
- The module must not calculate regime; calculate OMS; access Foundation.
...
|-----------|----------------|--------|---------|-------|
| Input Boundary | Accept and validate declared upstream contracts. | Overall Market Score. | Accepted input set or rejection reason | Exposure Engine |
| Assessment Boundary | Apply the approved module responsibility without expanding product scope. | Accepted input set | Intermediate assessment context | Exposure Engine |
| Explainability Boundary | Preserve reasons, source references, and dependency context. | Assessment context | Explainability metadata | Exposure Engine |
| Output Boundary | Produce Exposure Recommendation for downstream consumers. | Assessment context and explainability metadata | Exposure Recommendation | Exposure Engine |
| Consumer Boundary | Expose Exposure Recommendation only to approved downstream consumers. | Exposure Recommendation request | Exposure Recommendation or unavailable status | Exposure Engine |

---

## 8. Data Model

Exposure Engine defines or owns the Exposure Recommendation contract and consumes upstream contracts without mutating them.
...
Exposure Recommendation contract characteristics:

- Contract identity.
- Contract version.
- Evaluation timestamp or reference time.
- Upstream dependency references.
- Assessment value owned by Exposure Engine.
...
↓
Evaluate only the approved Exposure Engine responsibility
↓
Attach explainability and auditability metadata
↓
Publish Exposure Recommendation to approved downstream consumers
```
...
- Produce a single explainable Exposure Recommendation for downstream consumers.
...
- Exposure Recommendation or unavailable status.

Expected behavior:

- Serve only approved downstream consumers: Portfolio Engine.
...
1. Receive upstream input from approved dependency: Overall Market Score.
...
4. Execute only the approved Exposure Engine assessment responsibility.
5. Preserve reasons and upstream references for explainability.
6. Produce Exposure Recommendation with validation status and audit metadata.
7. Expose Exposure Recommendation to downstream consumers: Portfolio Engine.
...
- Allow downstream consumers to respect the absence of Exposure Recommendation.
...
- Exposure Recommendation must include reasons sufficient for downstream inspection.

Auditability:

- Exposure Recommendation must preserve upstream references and evaluation metadata for reproducibility.
...
- Validate dependency direction before producing Exposure Recommendation.
- Do not include implementation code in this Technical Design.

Acceptance Criteria:

- Exposure Engine declares upstream dependencies.
- Exposure Engine declares downstream consumers.
- Exposure Engine defines public interfaces.
- Exposure Engine defines internal workflow.
- Exposure Engine defines consumed and produced data contracts.
- Exposure Engine defines error handling.
- Exposure Engine preserves Regime First.
- Exposure Engine preserves Single Source of Truth.
- Exposure Engine preserves Separation of Concerns.
- Exposure Engine preserves Hard Gate Principle.
- Exposure Engine preserves Explainability.
- Exposure Engine preserves Auditability.
- Exposure Engine preserves approved dependency direction.
- Exposure Engine produces Exposure Recommendation only.
- Exposure Engine does not access Foundation directly.
- The module must not calculate regime; calculate OMS; access Foundation.
...
|---------|------|-------------|
| 1.0 | 2026-07-14 | Initial Technical Design for TD-015 Exposure Engine. |
