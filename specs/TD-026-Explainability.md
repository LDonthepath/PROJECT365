...
Define the Technical Design for TD-026 Explainability.

Explainability exists to display reasons behind every market assessment and recommendation.
...
This Technical Design covers Explainability as a Presentation component.
...
- Internal workflow.
- Data contract ownership for Explainability View.
...
- The module must preserve complete auditability.
...
Explainability occupies the TD-026 position in that chain and preserves Hard Gate Principle, Single Source of Truth, Separation of Concerns, Explainability, Auditability, approved dependency direction, and the Presentation business logic restriction.
...
Explainability consumes Governance Decision Records and produces Explainability View for approved downstream consumers.

Purpose:

- Display reasons behind every market assessment and recommendation.

Responsibilities:

- Accept only declared upstream contracts.
- Produce only Explainability View.
...
- Explainability does not own upstream contract creation.
- Explainability does not bypass downstream module ownership.
- Explainability does not access Foundation directly except through approved downstream paths and approved consumed outputs.
- Explainability does not modify immutable upstream outputs or governance records.
...
|-----------|----------------|--------|---------|-------|
| Input Boundary | Accept and validate declared upstream contracts. | Governance Decision Records. | Accepted input set or rejection reason | Explainability |
| Presentation Boundary | Apply the approved module responsibility without expanding product scope. | Accepted input set | Presentation context | Explainability |
| Read-Only Boundary | Preserve upstream records and prevent mutation or recalculation. | Presentation context | Read-only display context | Explainability |
| Explainability Boundary | Preserve reasons, source references, and dependency context. | Read-only display context | Explainability metadata | Explainability |
| Output Boundary | Produce Explainability View for downstream consumers. | Presentation context and explainability metadata | Explainability View | Explainability |
| Consumer Boundary | Expose Explainability View only to approved downstream consumers. | Explainability View request | Explainability View or unavailable status | Explainability |

---

## 8. Data Model

Explainability defines or owns the Explainability View contract and consumes upstream contracts without mutating them.
...
Explainability View contract characteristics:

- Contract identity.
- Contract version.
- View timestamp or reference time.
- Upstream dependency references.
- Presentation status owned by Explainability.
...
↓
Apply only the approved Explainability presentation responsibility
↓
Attach preserved explainability and auditability metadata
↓
Publish Explainability View to approved downstream consumers
```
...
- Produce a single read-only Explainability View output for downstream consumers.
...
- Explainability View or unavailable status.

Expected behavior:

- Serve only approved downstream consumers: TD-027 Historical Explorer.
...
1. Receive upstream input from approved dependency: TD-025 Inspector.
...
4. Execute only the approved Explainability presentation responsibility.
5. Preserve reasons and upstream references for explainability.
6. Produce Explainability View with validation status and audit metadata.
7. Expose Explainability View to downstream consumers: TD-027 Historical Explorer.
...
- Allow downstream consumers to respect the absence of Explainability View.
...
- Explainability View must preserve reasons sufficient for downstream inspection when such reasons are present in approved Governance records.

Auditability:

- Explainability View must preserve upstream references and evaluation metadata for reproducibility.
...
- Validate dependency direction before producing Explainability View.
- Do not include implementation code in this Technical Design.

Acceptance Criteria:

- Explainability declares upstream dependencies.
- Explainability declares downstream consumers.
- Explainability defines public interfaces.
- Explainability defines internal workflow.
- Explainability defines consumed and produced data contracts.
- Explainability defines error handling.
- Explainability preserves Single Source of Truth.
- Explainability preserves Separation of Concerns.
- Explainability preserves Hard Gate Principle.
- Explainability preserves Explainability.
- Explainability preserves Auditability.
- Explainability preserves approved dependency direction.
- Explainability produces Explainability View only.
- Explainability does not perform business logic.
- Explainability does not calculate market intelligence.
- Explainability does not change portfolio recommendations.
- Explainability does not modify governance records.
- Presentation only displays approved outputs.
- The module must preserve complete auditability.
...
|---------|------|-------------|
| 1.0 | 2026-07-14 | Initial Technical Design for TD-026 Explainability. |
