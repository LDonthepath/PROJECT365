...
Define the Technical Design for TD-017 Risk Engine.

Risk Engine exists to evaluate portfolio risk while remaining explainable and without changing Portfolio Assessment.
...
This Technical Design covers Risk Engine as a Portfolio Intelligence component.
...
- Internal workflow.
- Data contract ownership for Risk Assessment.
...
- The module must not change Portfolio Assessment; calculate market regime; access Foundation.
...
Risk Engine occupies the TD-017 position in that chain and preserves Regime First, Single Source of Truth, Separation of Concerns, Hard Gate Principle, Explainability, Auditability, and approved dependency direction.
...
Risk Engine consumes approved upstream outputs and produces Risk Assessment for approved downstream consumers.

Purpose:

- Evaluate portfolio risk while remaining explainable and without changing Portfolio Assessment.

Responsibilities:

- Accept only declared upstream contracts.
- Produce only Risk Assessment.
...
- Risk Engine does not own upstream contract creation.
- Risk Engine does not bypass downstream module ownership.
- Risk Engine does not access Foundation directly.
- Risk Engine does not modify immutable upstream outputs.
- The module must not change Portfolio Assessment; calculate market regime; access Foundation.
...
|-----------|----------------|--------|---------|-------|
| Input Boundary | Accept and validate declared upstream contracts. | Portfolio Assessment. | Accepted input set or rejection reason | Risk Engine |
| Assessment Boundary | Apply the approved module responsibility without expanding product scope. | Accepted input set | Intermediate assessment context | Risk Engine |
| Explainability Boundary | Preserve reasons, source references, and dependency context. | Assessment context | Explainability metadata | Risk Engine |
| Output Boundary | Produce Risk Assessment for downstream consumers. | Assessment context and explainability metadata | Risk Assessment | Risk Engine |
| Consumer Boundary | Expose Risk Assessment only to approved downstream consumers. | Risk Assessment request | Risk Assessment or unavailable status | Risk Engine |

---

## 8. Data Model

Risk Engine defines or owns the Risk Assessment contract and consumes upstream contracts without mutating them.
...
Risk Assessment contract characteristics:

- Contract identity.
- Contract version.
- Evaluation timestamp or reference time.
- Upstream dependency references.
- Assessment value owned by Risk Engine.
...
↓
Evaluate only the approved Risk Engine responsibility
↓
Attach explainability and auditability metadata
↓
Publish Risk Assessment to approved downstream consumers
```
...
- Produce a single explainable Risk Assessment for downstream consumers.
...
- Risk Assessment or unavailable status.

Expected behavior:

- Serve only approved downstream consumers: Cluster Rotation Engine.
...
1. Receive upstream input from approved dependency: Portfolio Assessment.
...
4. Execute only the approved Risk Engine assessment responsibility.
5. Preserve reasons and upstream references for explainability.
6. Produce Risk Assessment with validation status and audit metadata.
7. Expose Risk Assessment to downstream consumers: Cluster Rotation Engine.
...
- Allow downstream consumers to respect the absence of Risk Assessment.
...
- Risk Assessment must include reasons sufficient for downstream inspection.

Auditability:

- Risk Assessment must preserve upstream references and evaluation metadata for reproducibility.
...
- Validate dependency direction before producing Risk Assessment.
- Do not include implementation code in this Technical Design.

Acceptance Criteria:

- Risk Engine declares upstream dependencies.
- Risk Engine declares downstream consumers.
- Risk Engine defines public interfaces.
- Risk Engine defines internal workflow.
- Risk Engine defines consumed and produced data contracts.
- Risk Engine defines error handling.
- Risk Engine preserves Regime First.
- Risk Engine preserves Single Source of Truth.
- Risk Engine preserves Separation of Concerns.
- Risk Engine preserves Hard Gate Principle.
- Risk Engine preserves Explainability.
- Risk Engine preserves Auditability.
- Risk Engine preserves approved dependency direction.
- Risk Engine produces Risk Assessment only.
- Risk Engine does not access Foundation directly.
- The module must not change Portfolio Assessment; calculate market regime; access Foundation.
...
|---------|------|-------------|
| 1.0 | 2026-07-14 | Initial Technical Design for TD-017 Risk Engine. |
